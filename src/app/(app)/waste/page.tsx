...
import { IconWaste, IconEntry, IconCalendar } from "@/components/stat-icons";

export default function WastePage() {
  const [entries, setEntries] = useState<any[]>([]);
  const [products, setProducts] = useState<any[]>([]);
  const [formData, setFormData] = useState({ productId: "", quantity: "", reason: "", date: new Date().toISOString().split("T")[0] });
  const [submitting, setSubmitting] = useState(false);
  const [loading, setLoading] = useState(false);

  useEffect(() => { fetchProducts(); fetchEntries(); }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const res = await fetch("/api/waste", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(formData) });
    if (res.ok) { setFormData({ productId: "", quantity: "", reason: "", date: new Date().toISOString().split("T")[0] }); fetchEntries(); }
    setSubmitting(false);
  };

  const totalWaste = entries.reduce((sum, e) => sum + e.quantity, 0);
  const wasteToday = entries.filter(e => e.date?.slice(0, 10) === new Date().toISOString().slice(0, 10)).reduce((sum, e) => sum + e.quantity, 0);
  const reasons = ["Expired", "Damaged", "Overproduced", "Quality issue", "Other"];

  const statCards = [
    { label: "Total Wasted", value: totalWaste, icon: <IconWaste size={18} /> },
    { label: "Wasted Today", value: wasteToday, icon: <IconCalendar size={18} /> },
    { label: "Waste Entries", value: entries.length, icon: <IconEntry size={18} /> },
  ];

  return (
    <div className="min-h-full bg-[#f7f7f8]">
      <div className="flex items-center justify-between px-8 py-4 bg-white border-b border-gray-100">
        <div><h1 className="text-xl font-semibold text-gray-900">Waste</h1><p className="text-sm text-gray-400 mt-0.5">Track and reduce unsold or damaged items</p></div>
        ...
        <div className="grid grid-cols-3 gap-6">
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div><Label className="text-xs text-gray-500 mb-1 block">Quantity</Label>
                <Input type="number" placeholder="e.g. 12" value={formData.quantity} onChange={(e) => setFormData({ ...formData, quantity: e.target.value })} className="text-sm" required />
              </div>
              ...
              <Button type="submit" className="w-full bg-[#0f1117] hover:bg-[#1e2130] text-white text-sm" disabled={submitting}>{submitting ? "Saving..." : "Log Waste"}</Button>
            </form>
          </div>
          <div className="col-span-2 bg-white rounded-xl border border-gray-100 shadow-sm">
            ...
            {loading ? <div className="py-16 text-center text-sm text-gray-400">Loading...</div> : (
              <Table>
                <TableHeader><TableRow><TableHead className="text-xs">Date</TableHead><TableHead className="text-xs">Product</TableHead><TableHead className="text-xs">Reason</TableHead><TableHead className="text-xs text-right">Qty</TableHead></TableRow></TableHeader>
                <TableBody>
                  {entries.map((entry: any) => (
                    <TableRow key={entry.id}>
                      ...
                      <TableCell><span className="text-xs bg-gray-100 text-gray-600 px-2 py-1 rounded-md">{entry.reason || "—"}</span></TableCell>
                      <TableCell className="text-sm text-right font-mono text-gray-700">{entry.quantity}</TableCell>
                    </TableRow>
                  ))}
                  {entries.length === 0 && <TableRow><TableCell colSpan={4} className="text-center py-12 text-sm text-gray-400">No waste logged yet</TableCell></TableRow>}
                </TableBody>
              </Table>
            )}
          </div>
