...
import { IconProduction, IconBread, IconEntry } from "@/components/stat-icons";
...
export default function ProductionPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const res = await fetch("/api/production", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(formData) });
    if (res.ok) { setFormData({ productId: "", quantity: "", date: new Date().toISOString().split("T")[0] }); fetchEntries(); }
    setSubmitting(false);
  };

  const totalToday = entries.filter(e => e.date?.slice(0, 10) === new Date().toISOString().slice(0, 10)).reduce((sum, e) => sum + e.quantity, 0);

  const statCards = [
    { label: "Total Batches", value: entries.length, icon: <IconEntry size={18} /> },
    { label: "Produced Today", value: totalToday, icon: <IconProduction size={18} /> },
  ...
  return (
    <div className="min-h-full bg-[#f7f7f8]">
      <div className="flex items-center justify-between px-8 py-4 bg-white border-b border-gray-100">
        <div><h1 className="text-xl font-semibold text-gray-900">Production</h1><p className="text-sm text-gray-400 mt-0.5">Log daily baked quantities</p></div>
        ...
        <div className="grid grid-cols-3 gap-6">
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div><Label className="text-xs text-gray-500 mb-1 block">Quantity Baked</Label>
                <Input type="number" placeholder="e.g. 200" value={formData.quantity} onChange={(e) => setFormData({ ...formData, quantity: e.target.value })} className="text-sm" required />
              </div>
              ...
              <Button type="submit" className="w-full bg-[#0f1117] hover:bg-[#1e2130] text-white text-sm" disabled={submitting}>{submitting ? "Saving..." : "Log Production"}</Button>
            </form>
          </div>
            ...
            {loading ? <div className="py-16 text-center text-sm text-gray-400">Loading...</div> : (
              <Table>
                ...
                <TableBody>
                  {entries.map((e: any) => (
                    <TableRow key={e.id}>
                      <TableCell className="text-sm text-gray-500">{new Date(e.date).toLocaleDateString("en-GB")}</TableCell>
                      <TableCell className="text-sm font-medium text-gray-900">{e.product?.name}</TableCell>
                      <TableCell className="text-sm text-right font-mono text-gray-700">{e.quantity}</TableCell>
                    </TableRow>
                  ))}
                  {entries.length === 0 && <TableRow><TableCell colSpan={3} className="text-center py-12 text-sm text-gray-400">No production logged yet</TableCell></TableRow>}
                </TableBody>
              </Table>
            )}
          </div>
