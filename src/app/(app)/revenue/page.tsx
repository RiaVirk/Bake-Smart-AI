...
import { IconRevenue, IconSales, IconTrend } from "@/components/stat-icons";

const PRICE_MAP: Record<string, number> = {
  "Brötchen": 0.5, "Croissant": 1.8, "Kaiserbrötchen": 0.6, "Baguette": 2.5, "Laugenbrötchen": 0.7,
};
...
export default function RevenuePage() {
  ...
  useEffect(() => {
    setLoading(true);
    fetch("/api/sales").then(r => r.json()).then(data => { setSales(Array.isArray(data) ? data : []); setLoading(false); });
  }, []);
  ...
  const todayRevenue = sales.filter(s => s.date?.slice(0, 10) === new Date().toISOString().slice(0, 10)).reduce((sum, s) => sum + s.quantity * getPrice(s.product?.name), 0);
  ...
  const statCards = [
    { label: "Total Revenue", value: fmt(totalRevenue), icon: <IconRevenue size={18} /> },
    { label: "Today's Revenue", value: fmt(todayRevenue), icon: <IconTrend size={18} /> },
    { label: "Total Sales", value: sales.length, icon: <IconSales size={18} /> },
  ];

  return (
    <div className="min-h-full bg-[#f7f7f8]">
      <div className="flex items-center justify-between px-8 py-4 bg-white border-b border-gray-100">
        <div><h1 className="text-xl font-semibold text-gray-900">Revenue</h1><p className="text-sm text-gray-400 mt-0.5">Estimated earnings from sales</p></div>
      ...
      <div className="px-8 py-6 space-y-6">
              ...
              <div className="flex items-end gap-3 h-36">
                {last7.map(([date, rev]) => (
                  <div key={date} className="flex-1 flex flex-col items-center gap-1.5">
                    ...
                    <div className="w-full bg-[#0f1117] rounded-t-md" style={{ height: `${(rev / maxRev) * 100}%`, minHeight: "4px" }} />
                    <span className="text-xs text-gray-400">{new Date(date).toLocaleDateString("en-GB", { weekday: "short" })}</span>
                  </div>
                ))}
              </div>
            )}
          ...
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
            ...
            {Object.keys(byProduct).length === 0 ? (
              ...
              <div className="space-y-3">
                {Object.entries(byProduct).sort((a, b) => b[1].revenue - a[1].revenue).map(([name, { qty, revenue }]) => (
                  <div key={name}>
                    ...
                    <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden">
                      <div className="h-full bg-[#0f1117] rounded-full" style={{ width: `${(revenue / totalRevenue) * 100}%` }} />
                    </div>
        ...
        <div className="bg-amber-50 border border-amber-100 rounded-xl px-5 py-3 text-sm text-amber-700">
          Revenue is estimated using default prices per product. Configure actual prices in Settings.
        </div>
      </div>
    </div>
  );
